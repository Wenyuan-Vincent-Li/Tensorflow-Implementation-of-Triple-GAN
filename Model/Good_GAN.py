import sys, os
if os.getcwd().endswith("Tensorflow-Implementation-of-Triple-GAN"):
    root_dir = os.getcwd()
else:
...
class Good_GAN(model_base.NN_Base):
    def __init__(self, config):
        super(Good_GAN, self).__init__(config.BATCH_NORM_DECAY, config.BATCH_NORM_EPSILON)
        self.config = config

    def good_generator(self, z, y, reuse = False):
        with tf.variable_scope("good_generator") as scope:
            if reuse:
            ...
            if self.config.DATA_NAME == "mnist":
                ...
                h2 = self._WN_deconv2d(h1, 3, k_w=5, k_h=5, d_w=2, d_h=2, init_scale= 0.1, init = False, name='gg_wndconv0')
