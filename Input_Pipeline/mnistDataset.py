...
class mnistDataset(object):
    ...
    def get_filenames(self):
        ...
        if self.subset == 'train':
            return [os.path.join(self.data_dir, 'mnist_%s_%s.tfrecords'%(self.subset, str(self.num_label).zfill(6))),
                    os.path.join(self.data_dir, 'mnist_%s_%s.tfrecords'%(self.subset, \
                                                                        str(self.train_size - self.num_label).zfill(6)))]

        else:
            return [os.path.join(self.data_dir, 'mnist_%s.tfrecords'%self.subset)]
    ...
    def input_from_tfrecord_filename(self):
        ...
        if self.subset == 'test':
            ...
            dataset = [tf.data.TFRecordDataset(filename[0]), tf.data.TFRecordDataset(filename[0]),
                       tf.data.TFRecordDataset(filename[1])]
        return dataset
    ...
    def parser(self, serialized_example):
        ...
        # image.set_shape([self.config.IMAGE_HEIGHT, self.config.IMAGE_WIDTH, 1])
...
def test_main_train_val():
    ...
    num_label = 200
    train_data = mnistDataset(data_dir, tmp_config, num_label, 'train', True)
    val_data = mnistDataset(data_dir, tmp_config, None, 'test', False)
...
def test_val():
    ...
    val_data = mnistDataset(data_dir, tmp_config, None, 'test')
...
def test_customized():
    ...
    data = mnistDataset(data_dir, tmp_config, None, 'test', False)
    filename_list = ["mnist_train_059980.tfrecords"]
